...
import { emailVerification_table, passwordVerification_table } from './db/schema/users';
...
export const generatePasswordResetToken = async (userId: string) => {
	const storedUserTokens = await db
		...
		.where(eq(passwordVerification_table.userId, userId));
	if (storedUserTokens.length > 0) {
		const reusableStoredToken = storedUserTokens.find((token) => {
			// check if expiration is within 1 hour
			// and reuse the token if true
	...
	const token = generateRandomString(63);
	await db.insert(passwordVerification_table).values({
		id: token,
		expires: (new Date().getTime() + EXPIRES_IN) as unknown as bigint,
		userId: userId
	});
...
export const validatePasswordResetToken = async (token: string) => {
	const storedToken = await db.transaction(async (trx) => {
		const [storedToken] = await trx
			.select()
			.from(passwordVerification_table)
			.where(eq(passwordVerification_table.id, token));
		if (!storedToken) throw new Error('Invalid token');
		await trx.delete(passwordVerification_table).where(eq(passwordVerification_table.id, token));
		return storedToken;
	});
	const tokenExpires = Number(storedToken.expires); // bigint => number conversion
	if (!isWithinExpiration(tokenExpires)) {
